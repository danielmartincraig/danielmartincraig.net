...
* [`src/danielmartincraig/`](src/danielmartincraig/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/danielmartincraig/core.cljs): contains the SPA entry point, `init`
* [`test/danielmartincraig/`](test/danielmartincraig/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `danielmartincraig` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/danielmartincraig/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns danielmartincraig.example
  (:require [danielmartincraig.config :as config])
